...
// Stored in localStorage as 'hc-primary' (hex like '#7C3AED').
(function () {
    const STORAGE_KEY = 'hc-primary';
    ...
    function applyPrimary(hex) {
        ...
        root.style.setProperty('--hc-primary', hex);
        root.style.setProperty('--hc-primary-strong', darken(hex, 0.15));
        root.style.setProperty('--hc-primary-emphasis', darken(hex, 0.30));
        root.style.setProperty('--hc-primary-soft', softTint(hex, 0.18));
        root.style.setProperty('--hc-primary-tint', softTint(hex, 0.08));
    }
    ...
    window.hcTheme = {
        ...
        reset: function () {
            ...
            root.style.removeProperty('--hc-primary-strong');
            root.style.removeProperty('--hc-primary-emphasis');
            root.style.removeProperty('--hc-primary-soft');
            root.style.removeProperty('--hc-primary-tint');
        }
    };

    // Apply on load
    const saved = window.hcTheme.get();
