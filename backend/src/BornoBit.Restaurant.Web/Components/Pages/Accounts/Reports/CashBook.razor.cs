...
public partial class CashBook : ComponentBase
{
    ...
    private async Task ReloadAsync()
    {
        ...
        try
        {
            _report = await Mediator.Send(new GetCashLedgerQuery(_from, _to, _accountFilter));
        }
        catch (Exception ex) { _error = $"Failed to load cash book: {ex.Message}"; }
    ...
    private Task OnAccountChanged(CashAccountDto? a) { _accountFilter = a?.Id; return ReloadAsync(); }
}
