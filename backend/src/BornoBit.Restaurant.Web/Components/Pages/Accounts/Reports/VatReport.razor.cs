...
public partial class VatReport : ComponentBase
{
    ...
    private VatReportDto _report = new(Array.Empty<VatReportRowDto>(), 0m, 0m, "Tk");
    ...
    private async Task ReloadAsync()
    {
        ...
        if (_from > _to)
        {
            ...
            _report = new VatReportDto(Array.Empty<VatReportRowDto>(), 0m, 0m, "Tk");
            return;
        }
        ...
        try
        {
            _report = await Mediator.Send(new GetVatReportQuery(_from, _to));
        }
        catch (Exception ex) { _error = $"Failed to load VAT report: {ex.Message}"; }
